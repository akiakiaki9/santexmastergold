// src/app/catalog/CatalogContent.js
'use client'
import React, { useState, useCallback, useEffect } from 'react';
import { useSearchParams, useRouter } from 'next/navigation';
import Link from 'next/link';
import {
    FiGrid, FiList, FiBox, FiGlobe, FiX, FiChevronRight, FiSearch,
    FiExternalLink, FiInstagram, FiFacebook, FiYoutube, FiChevronDown,
    FiShield, FiTruck, FiStar, FiPackage
} from 'react-icons/fi';
...
const ProductCard = ({ product }) => {
    ...
    return (
        <div className="product-card-large no-link">
            <div className="product-card-link-static">{content}</div>
        </div>
    );
};
...
const BrandCard = ({ brand, isActive, onClick }) => {
    return (
        <div className={`brand-card ${isActive ? 'active' : ''}`} onClick={() => onClick(brand)}>
            {brand.image && (
        ...
        <div className="brand-social-widget">
            ...
            <div className="brand-social-widget-links">
                {brandInfo.social_links.map((link, index) => (
                    <a key={index} href={link.url} target="_blank" rel="noopener noreferrer" className="social-widget-link">
                ...
                <div className="container">
                    ...
                    <div className="brands-section">
                        <div className="brands-header">
                            ...
                            <button className="toggle-brands-btn" onClick={() => setShowBrandsList(!showBrandsList)}>
                                ...
                                <>
                                    <div className="brand-sidebar-info">
                                        ...
                                        <div className="brand-features">
                                            <div className="brand-feature"><FiShield /><span>Официальная гарантия</span></div>
                                            <div className="brand-feature"><FiTruck /><span>Прямые поставки</span></div>
                                            <div className="brand-feature"><FiStar /><span>Сертифицировано</span></div>
                                        </div>
                                    </div>
                        ...
                        <div className="catalog-products-area">
                            ...
                            {filteredProducts.length === 0 ? (
                                <div className="no-products">
                                    ...
                                    <button onClick={clearFilters} className="reset-filters-btn">Сбросить все фильтры</button>
                                </div>
                            ) : (
                                <>
                    ...
                    <div className="mobile-filters-overlay" onClick={() => setShowMobileFilters(false)}>
                        <div className="mobile-filters-panel" onClick={e => e.stopPropagation()}>
                            <div className="mobile-filters-header">
                                <h3>Категории</h3>
                                <button onClick={() => setShowMobileFilters(false)}><FiX /></button>
                            </div>
